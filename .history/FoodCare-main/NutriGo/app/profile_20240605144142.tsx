...
import { View, Text, StyleSheet, Image, TextInput, TouchableOpacity, ScrollView, Animated, Alert } from 'react-native';
...
const Profile: React.FC = () => {
    ...
    if (!result.canceled && result.assets && result.assets.length > 0) {
      ...
      if (FileSystem.documentDirectory) {
        ...
        try {
          ...
          console.log('Image copied to:', newPath);  // Debug log
          setImageUri(newPath);
          ...
          Alert.alert('Error', 'Error copying the file: ' + error.message);
        }
      } else {
        console.error('FileSystem.documentDirectory is null');
        Alert.alert('Error', 'FileSystem.documentDirectory is null');
      }
    }
  };
  ...
  console.log('User Image URI:', localUser?.image);  // Debug log
  ...
  return (
    <ScrollView contentContainerStyle={styles.scrollContent}>
      ...
      {imageUri && <Text>Image URI: {imageUri}</Text>} {/* Display the image URI for debugging */}
    </ScrollView>
  );
};
