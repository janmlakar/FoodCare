  ...
  const renderLabel = useCallback(({ item, setLabels, labels }: { item: string, setLabels: React.Dispatch<React.SetStateAction<string[]>>, labels: string[] }) => (
    ...
    >
      {labels.includes(item) ? (
        ...
        >
          <Text style={localStyles.labelButtonTextSelected} numberOfLines={1} ellipsizeMode="tail">
            {item}
          </Text>
        </LinearGradient>
      ) : (
...
const localStyles = StyleSheet.create({
  ...
  labelButton: {
    ...
    paddingHorizontal: 15,
    borderRadius: 20,
    backgroundColor: 'lightgray', // Light gray background for unselected state
    marginHorizontal: 5,
    marginVertical: 5,
    maxWidth: '100%',
  },
  selectedLabelButton: {
    // Empty to use gradient background dynamically
  },
  gradientBackground: {
    borderRadius: 20,
    paddingVertical: 5,
